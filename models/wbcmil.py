...
class WBCMIL(BaseAggregator):
    def __init__(self, input_dim, num_classes, multi_attention=True, attention_latent_dim=256):
        super(BaseAggregator, self).__init__()

        self.projection = nn.Sequential(nn.Linear(input_dim, 512, bias=True), nn.ReLU())
        self.mil_latent_dim = 512
    ...
    def forward(self, x):
        ...
        features = self.projection(x)  # Uncomment if needed: self.mil_encoder(x.squeeze().unsqueeze(dim=2))
        ...
        if self.multi_attention:
            ...
            # }
            return bag_feature_stack, prediction
