...
class Perceiver(BaseAggregator):
    def __init__(
        ...
        *,
        ...
        self._fc1 = nn.Sequential(nn.Linear(self.input_dim, 512), nn.ReLU())  # adapted to 2048 from RetCCL features

        self.input_dim = 512
        self.fourier_encode_data = fourier_encode_data
        fourier_channels = (input_axis * ((num_freq_bands * 2) + 1)) if fourier_encode_data else 0
        self.input_dim = fourier_channels + self.input_dim

        self.latents = nn.Parameter(torch.randn(num_latents, latent_dim))

        get_cross_attn = lambda: PreNorm(
            latent_dim,
            PerceiverAttention(latent_dim, self.input_dim, heads=cross_heads, dim_head=cross_dim_head, dropout=attn_dropout),
        ...
        self.to_logits = (
            nn.Sequential(Reduce("b n d -> b d", "mean"), nn.LayerNorm(latent_dim), nn.Linear(latent_dim, 64), nn.ReLU(), nn.Linear(64, num_classes))
